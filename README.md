...
I am planning on changing to the swedish [_Kvikk layout_](https://lykt.xyz/skl/kvikk/), which has been optimized and evaluated with Carplax optimization. The numbers are fine for both english and swedish. I will begin by adding an alpha layer called ALPHA/1, which can be switched to from the MEDIA-layers rightmost-key on the homerow, the one sandwiched between the os-toggle key and the lock-key.    
